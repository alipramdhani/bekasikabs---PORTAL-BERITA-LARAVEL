...
<body>
    ...
    <section id="section-2">
        <div class="row p-5">
            <!-- News 2 -->
            <div id="pemerintah" class="col p-3">
                <h3 class="mx-3 mb-5">Pemerintah</h3>
                @php $counter = 0; @endphp
                @foreach ($pemerintahans as $pemerintah)
                    @php
                        $limitedHeadline = Str::limit($pemerintah->judul_berita, 60);
                        $limitedcontent = Str::limit($pemerintah->isi, 230);
                    @endphp
                    <div class="terkini-item d-flex my-5">
                        <a href="{{ route('detailBerita.show', $pemerintah->id_berita) }}">
                            <div class="mx-3">
                                <img width="400" height="250" class="rounded-4" src="{{ asset('storage/uploads/dataBerita/'.$pemerintah->gambar) }}" alt="terkini Image">
                            </div>
                        </a>
                        <div class="mx-5">
                            <p style="color: #fe7d21;" class="fs-6 lh-lg">
                            {{ \Carbon\Carbon::parse($pemerintah->tanggal)->format('d F Y') }}
                            </p>
                            <a href="{{ route('detailBerita.show', $pemerintah->id_berita) }}" class="fs-4 headline-section-2 text-wrap">
                                {{ $limitedHeadline }}
                            </a>
                            ...
                            <a class="selengkapnya text-end fs-6 my-3 py-2 px-3 rounded-3" href="{{ route('detailBerita.show', $pemerintah->id_berita) }}">
                                Selengkapnya > 
                            </a>
